...
class PairTest {
    ...
    void isSatisfiedSome() throws CardParseException {
        Hand hand0 = parser.parse(": 2H 2H 3H 4H 5H");

        Optional<Hand> handOptional = new Pair().isSatisfied(hand0);

        Assert.assertTrue(handOptional.isPresent());
        Assert.assertEquals(handOptional.get().getCards().size(), 2);
    }
    ...
    void evaluateWIN() throws CardParseException {
        ...
        Outcome outcome = new Pair().evaluate(hand0, hand1);
    ...
    void evaluateBothTIE() throws CardParseException {
        Hand hand0 = parser.parse(": 2H 2H 3H 4H 5H");
        Hand hand1 = parser.parse(": 2H 2H 3H 4H 5H");
